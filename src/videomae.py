...
from utils import Block, PatchEmbedSwin, get_sinusoid_encoding_table
...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.patch_embed = PatchEmbedSwin(
    ...
    def forward_features(self, x, mask):
        ...
        B, _, C = x.shape
        x_vis = x[~(mask>0)].reshape(B, -1, C) # ~mask means visible
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask):
        _, _, T, _, _ = x.shape
        x_invis = rearrange(x,'b c (t p0) (h p1) (w p2) -> b (t h w) (p0 p1 p2 c)',p0=2,p1=16,p2=16)
        B,_,C = x_invis.shape
        x_invis = x_invis[(mask>0)].reshape(B,-1,C)
        ...
        pos_emd_vis = expand_pos_embed[~(mask>0)].reshape(B, -1, C)
        pos_emd_mask = expand_pos_embed[(mask>0)].reshape(B, -1, C)
        x_full = torch.cat([x_vis + pos_emd_vis, self.mask_token + pos_emd_mask], dim=1) # [B, N, C_d]
        x = self.decoder(x_full, pos_emd_mask.shape[1]) # [B, N_mask, 3 * 16 * 16]

        loss = self.loss_func(input=x, target=x_invis)
        return x, loss
