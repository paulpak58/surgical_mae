...
train_labels_80, train_num_each_80, train_start_vidx,\
    ...
    test_labels_80, test_num_each_80, test_start_vidx = get_data('/saiil2/paulpak/surgical_ncp/train_val_paths_labels1.pkl')
    # test_labels_80, test_num_each_80, test_start_vidx = get_data('./train_val_paths_labels1.pkl')

'''
with open("./logs/features/vitmae-cholec80/mae_features_train.pkl", 'rb') as f:
    mae_features_train = pickle.load(f)
with open("./logs/features/vitmae-cholec80/mae_features_val.pkl", 'rb') as f:
    mae_features_val = pickle.load(f)
with open("./logs/features/vitmae-cholec80/mae_features_test.pkl", 'rb') as f:
    mae_features_test = pickle.load(f)
'''
with open("./logs/features/vitmae-cholec80/mae_features_train_mean.pkl", 'rb') as f:
    mae_features_train = pickle.load(f)
with open("./logs/features/vitmae-cholec80/mae_features_val_mean.pkl", 'rb') as f:
    mae_features_val = pickle.load(f)
with open("./logs/features/vitmae-cholec80/mae_features_test_mean.pkl", 'rb') as f:
    mae_features_test = pickle.load(f)

print("load completed")

print("mae_features_train shape:", mae_features_train.shape)
print("mae_features_val shape:", mae_features_val.shape)
print("mae_features_test shape:", mae_features_test.shape)
...
for epoch in range(max_epochs):
    ...
    for i in train_we_use_start_idx_80:
        ...
        long_feature = get_long_feature(start_index=train_start_vidx[i],
                                        lfb=mae_features_train, LFB_length=train_num_each_80[i])
    ...
    with torch.no_grad():
        for i in val_we_use_start_idx_80:
            ...
            long_feature = get_long_feature(start_index=val_start_vidx[i],
                                            lfb=mae_features_val, LFB_length=val_num_each_80[i])
    ...
    with torch.no_grad():
        for i in test_we_use_start_idx_80:
            ...
            long_feature = get_long_feature(start_index=test_start_vidx[i],
                                            lfb=mae_features_test, LFB_length=test_num_each_80[i])
...
pred_name = model_name + '_test' + '_length30' + "_weights" + '.pkl'
if not os.path.exists("./logs/eval/tcn_mae/"):
    os.makedirs("./logs/eval/tcn_mae/")
pkl_dir = "./logs/eval/tcn_mae/" + pred_name
with open(pkl_dir, 'wb') as f:
    pickle.dump(best_test_all_preds_phase, f)
torch.save(best_model_wts, "./logs/eval/tcn_mae/" + model_name + "_weights" + ".pth")
