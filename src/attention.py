...
def cal_rel_pos_spatial(
    ...
    # print('Qshape,Kshape', q_shape, k_shape, attn.shape)
    # print('Distance h',dist_h)
...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x, thw_shape, scalars=None):
        B, N, _ = x.shape
        ...
        if self.rel_pos_temporal:
            if scalars is not None:
                upper = (torch.sort(scalars,descending=True)[0])
                lower = (-1*scalars[1:])

                # TODO: Scaled relative temporal positional embeddings
                '''
                scaled_embeddings = torch.cat((upper,lower),dim=0)
                scaled_embeddings = scaled_embeddings.unsqueeze(-1).repeat(1,self.rel_pos_t.shape[1])
                scaled_embeddings = scaled_embeddings.type_as(self.rel_pos_t).to(self.rel_pos_t.device) 
                self.rel_pos_t = nn.Parameter(self.rel_pos_t + scaled_embeddings)
                '''
...
class MultiScaleBlock(nn.Module):
    ...
    def forward(self, x, thw_shape=None, scalars=None):
        x_norm = self.norm1(x)
        x_block, thw_shape_new = self.attn(x_norm, thw_shape, scalars)
...
################################################################
# Helper functions implemented from
# https://github.com/facebookresearch/SlowFast/blob/main/slowfast/models/common.py 
################################################################
...
# --------------------------------------------------------
# Attention Head with Masked Decoder Projection
# References:
# https://github.com/facebookresearch/SlowFast/blob/main/slowfast/models/head_helper.py
# --------------------------------------------------------
