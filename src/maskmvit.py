...
from utils import calc_mvit_feature_geometry,HOGLayerC,TransformerBasicHead
from attention import MSSeparateHead
...
class MaskMViT(MViT):
    def __init__(self, cfg):
        ...
        del self.norm
        if hasattr(self,"TransformerBasicHead"):
            # del self.head
            del self.class_head
        ...
        # VIT_B layers less than default pretrain depth
        if self.pretrain_depth[0]>=len(self.feat_size):
            self.pretrain_depth[0]=len(self.feat_size)-1
    ...
    def forward(self, x, mask, return_all=False):
        '''
        if len(x) > 1:
            x, meta, mask = x
        else:
...
def build_cfg_maskfeat(size='small'):
    cfg = CfgNode()
    ...
    cfg.MASK.PRETRAIN_DEPTH = [15]
    cfg.MASK.HEAD_TYPE = "separate"
    cfg.MASK.PRED_HOG = True
    cfg.MASK.TIME_STRIDE_LOSS = True
    cfg.MASK.DECODER_EMBED_DIM = 512
    cfg.MASK.DECODER_SEP_POS_EMBED = False
    cfg.MASK.MAE_ON = False
    cfg.MASK.SCALE_INIT_BY_DEPTH = False
    cfg.MASK.PER_FRAME_MASKING = False
    cfg.MASK.MAE_RND_MASK = False
    cfg.MASK.NORM_PRED_PIXEL = True
    cfg.MASK.TIME_STRIDE_LOSS = True
    cfg.MASK.DEC_KV_KERNEL = []
    cfg.MASK.DEC_KV_STRIDE = []
    ...
    cfg.MVIT.POOL_Q_STRIDE = [[0, 1, 1, 1], [1, 1, 2, 2], [2, 1, 1, 1], [3, 1, 2, 2], [4, 1, 1, 1], [5, 1, 1, 1], [6, 1, 1, 1], [7, 1, 1, 1], [8, 1, 1, 1], [9, 1, 1, 1], [10, 1, 1, 1], [11, 1, 1, 1], [12, 1, 1, 1], [13, 1, 1, 1], [14, 1, 1, 1], [15, 1, 1, 1]]
    cfg.MVIT.POOL_KVQ_KERNEL = [3,3,3]
    cfg.MVIT.POOL_KV_STRIDE_ADAPTIVE = [1,8,8]
    cfg.MVIT.DROPPATH_RATE = 0.0
    cfg.MVIT.CLS_EMBED_ON = True    # default: True
    cfg.MVIT.USE_ABS_POS = False    # default: True
    cfg.MVIT.USE_FIXED_SINCOS_POS = False
    cfg.MVIT.SEP_POS_EMBED = True   # default: False
    cfg.MVIT.REL_POS_SPATIAL = True # default: False
    cfg.MVIT.REL_POS_TEMPORAL = True# default: False
    ...
    # OTHER CONFIGS
    cfg.AUG.ENABLE = True
    cfg.AUG.MASK_TUBE = False
    cfg.AUG.MASK_FRAMES = False
    cfg.AUG.MASK_WINDOW_SIZE = [8,7,7]
    cfg.AUG.MASK_RATIO = 0.4
    cfg.VIS_MASK.ENABLE = False
...
def build_cfg(pretrain=True,mask_model="",backbone="",default_cfg=None):
    if mask_model=='maskfeat':
        if pretrain:
            if backbone=='mvitv2_S':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/k400_MVITv2_S_16x4_MaskFeat_PT.yaml"
            elif backbone=='mvitv2_B':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/k400_MVITv2_B_32x3_MaskFeat_PT.yaml"
            elif backbone=='mvitv2_L':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/k400_MVITv2_L_16x4_MaskFeat_PT.yaml"
            elif backbone=='vit_B':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/in1k_VIT_B_MaskFeat_PT.yaml"
            elif backbone=='vit_L':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/in1k_VIT_L_MaskFeat_PT.yaml"
        else:
            if backbone=='mvitv2_S':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/k400_MVITv2_S_16x4_FT.yaml"
            elif backbone=='mvitv2_L':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/k400_MVITv2_L_16x4_FT.yaml"
            elif backbone=='vit_B':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/in1k_VIT_B_MaskFeat_FT.yaml"
            elif backbone=='vit_L':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/in1k_VIT_L_MaskFeat_FT.yaml"
    elif mask_model=='mae' or mask_model=='videomae':
        if pretrain:
            if backbone=='vit_B':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/k400_VIT_B_16x4_MAE_PT.yaml"
            elif backbone=='vit_L':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/k400_VIT_L_16x4_MAE_PT.yaml"
        else:
            if backbone=='vit_B':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/k400_VIT_B_16x4_FT.yaml"
            elif backbone=='vit_L':
                url = "https://raw.githubusercontent.com/facebookresearch/SlowFast/main/configs/masked_ssl/k400_VIT_L_16x4_FT.yaml"
    # req = requests.get(url)
    source = "/saiil2/paulpak"
    filename = url.split("/")[-1]
    # filepath = "./config/" + filename
    filepath = source + "/surgical_adventure/final_code/src/config/" + filename
...
if __name__=='__main__':
    # Pretrain Maskfeat models
    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=True,mask_model="maskfeat",backbone="mvitv2_S",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=True,mask_model="maskfeat",backbone="mvitv2_L",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=True,mask_model="maskfeat",backbone="vit_B",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=True,mask_model="maskfeat",backbone="vit_L",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    # Finetune Maskfeat models
    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=False,mask_model="maskfeat",backbone="mvitv2_S",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=False,mask_model="maskfeat",backbone="mvitv2_L",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=False,mask_model="maskfeat",backbone="vit_B",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=False,mask_model="maskfeat",backbone="vit_L",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    # Pretrain MAE models
    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=True,mask_model="mae",backbone="vit_B",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=True,mask_model="mae",backbone="vit_L",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    # Finetune MAE models
    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=False,mask_model="mae",backbone="vit_B",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')

    default_cfg = get_cfg()
    cfg = build_cfg(pretrain=False,mask_model="mae",backbone="vit_L",default_cfg=default_cfg)
    model = MaskMViT(cfg)
    print('done')
